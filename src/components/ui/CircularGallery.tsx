...
class Title {
    ...
    constructor({ gl, plane, renderer, text, textColor = '#ffffff', font = 'bold 40px sans-serif' }: any) {
        autoBind(this);
        this.gl = gl;
        this.plane = plane;
...
class Media {
    ...
    onResize({ screen, viewport }: any = {}) {
        ...
        // Responsive sizing based on screen width - all sizes reduced to half
        const isMobile = this.screen.width < 768;
        const isTablet = this.screen.width >= 768 && this.screen.width < 1024;

        // Adjust scale factors for different screen sizes (all halved)
        let heightFactor = 450; // was 900
        let widthFactor = 350;  // was 700

        if (isMobile) {
            // Smaller cards for mobile
            heightFactor = 300;  // was 600
            widthFactor = 225;   // was 450
        } else if (isTablet) {
            // Medium cards for tablet
            heightFactor = 375;  // was 750
            widthFactor = 287.5; // was 575
        }
